...
# At least 10 seconds between super mobs (milliseconds)
SUPER_MOB_MIN_INTERVAL = 10000
# Initialize to allow the first mob to appear
LAST_SUPER_MOB_TIME = -SUPER_MOB_MIN_INTERVAL
SUPER_MOB_PROBABILITY = 0.2     # 20% chance to spawn a super mob when eligible

# Difficulty levels
DIFFICULTY_LEVELS = [
    # Level 1: Letters
    list("ASDFGHJKLQWERTYUIOPZXCVBNM"),
    # Level 2: Letters + Numbers
    list("ASDFGHJKLQWERTYUIOPZXCVBNM") + list("1234567890"),
    # Level 3: Letters + Numbers + Symbols
    list("ASDFGHJKLQWERTYUIOPZXCVBNM") + list("1234567890!@#$%^&*()"),
    # Level 4: 2-letter combinations
    [''.join(p) for p in product("ASDFGHJKLQWERTYUIOPZXCVBNM", repeat=2)],
    # Level 5: 3-character combinations
    [''.join(p)
     for p in product("ASDFGHJKLQWERTYUIOPZXCVBNM1234567890", repeat=3)],
]
    ...
    def draw(self, surface, current_time):
        if not self.defeated:
            ...
            key_text_rect = key_text.get_rect(
                center=(self.x + MOB_SIZE / 2, self.y - 30))
            surface.blit(key_text, key_text_rect)
            # Render the timer below the mob
            elapsed_time = current_time - self.spawn_time
            time_remaining = max(
                0, (self.time_limit - elapsed_time) // 1000 + 1)
            timer_text = FONT_SMALL.render(
                f"{time_remaining}", True, HEART_COLOR)
            timer_text_rect = timer_text.get_rect(
                center=(self.x + MOB_SIZE / 2, self.y + MOB_SIZE + 15))
...
def create_mob(difficulty_level, spawn_time, current_time):
    ...
    if time_since_last_super >= SUPER_MOB_MIN_INTERVAL and random.random() < SUPER_MOB_PROBABILITY:
        ...
        key_list = DIFFICULTY_LEVELS[min(
            difficulty_level, len(DIFFICULTY_LEVELS)-1)]
...
def main():
    ...
    while running:
        ...
        if current_time - last_mob_spawn_time >= NEW_MOB_INTERVAL:
            new_mob = create_mob(player.difficulty_level,
    ...
    final_score_text = FONT.render(
        f"Final Score: {player.points}", True, TEXT_COLOR)
